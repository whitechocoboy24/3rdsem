...
if (isset($_POST['filter_orders'])) {
    ...
    $day = $_POST['day'] ?? '';
    if ($month != "" && $year != "" && $day != "") {
        $query = "SELECT * FROM orders WHERE status = 'delivered' AND MONTH(order_date) = '$month' AND YEAR(order_date) = '$year' AND DAY(order_date) = '$day'";
    } elseif ($month != "" && $year != "") {
        $query = "SELECT * FROM orders WHERE status = 'delivered' AND MONTH(order_date) = '$month' AND YEAR(order_date) = '$year'";
    } elseif ($month != "") {
        $query = "SELECT * FROM orders WHERE status = 'delivered' AND MONTH(order_date) = '$month'";
    } elseif ($year != "") {
        $query = "SELECT * FROM orders WHERE status = 'delivered' AND YEAR(order_date) = '$year'";
    } else {
        $query = "SELECT * FROM orders WHERE status = 'delivered'";
    }
} elseif (isset($_POST['search'])) {
    $search = $_POST['search'];
    $query = "SELECT * FROM orders WHERE status = 'delivered' AND (orders LIKE '%$search%' OR pickup_address LIKE '%$search%')";
} else {
    $query = "SELECT * FROM orders WHERE status = 'delivered'";
}
    ...
    foreach ($unique_user_ids as $user_id) {
        ...
        $orders_table .= "<td>
            <form action='' method='post' class='update-status-form'>
                ...
                <button type='submit' name='update_status_button' value='paid'>paid</button>
            </form>
        </td>";
        $orders_table .= "</tr>";
    }
