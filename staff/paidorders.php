...
if (isset($_POST['filter_orders'])) {
    ...
    $day = $_POST['day'] ?? '';
    if ($month != "" && $year != "" && $day != "") {
        $query = "SELECT * FROM orders WHERE status = 'paid' AND MONTH(order_date) = '$month' AND YEAR(order_date) = '$year' AND DAY(order_date) = '$day'";
    } elseif ($month != "" && $year != "") {
        $query = "SELECT * FROM orders WHERE status = 'paid' AND MONTH(order_date) = '$month' AND YEAR(order_date) = '$year'";
    } elseif ($month != "") {
        $query = "SELECT * FROM orders WHERE status = 'paid' AND MONTH(order_date) = '$month'";
    } elseif ($year != "") {
        $query = "SELECT * FROM orders WHERE status = 'paid' AND YEAR(order_date) = '$year'";
    } else {
        $query = "SELECT * FROM orders WHERE status = 'paid'";
    }
} elseif (isset($_POST['search'])) {
    $search = $_POST['search'];
    $query = "SELECT * FROM orders WHERE status = 'paid' AND (orders LIKE '%$search%' OR pickup_address LIKE '%$search%')";
} else {
    $query = "SELECT * FROM orders WHERE status = 'paid'";
}
...
if ($result->num_rows >  0) {
    ...
    $orders_table = "<table>";
    $orders_table .= "<tr><th>Order ID</th><th>Price</th><th>Products</th><th>Wash Method</th><th>Pickup Address</th><th>Delivery address</th><th>Order Date</th></tr>";
    $order_ids = array();
    $rows = array();
    $products = array();
            ...
            <div class="head-title">
                <div class="left">
                    ...
                    <ul class="breadcrumb">
                        ...
                        <li>
                            <a class="active" href="paidorders.php">Orders History</a>
                        </li>
                    </ul>
                </div>
