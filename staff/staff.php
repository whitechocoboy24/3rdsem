...
if (isset($_POST['filter_orders'])) {
    ...
    $day = $_POST['day'] ?? '';
    if ($month != "" && $year != "" && $day != "") {
        $query = "SELECT * FROM orders WHERE status = 'pending' AND MONTH(order_date) = '$month' AND YEAR(order_date) = '$year' AND DAY(order_date) = '$day'";
    } elseif ($month != "" && $year != "") {
        $query = "SELECT * FROM orders WHERE status = 'pending' AND MONTH(order_date) = '$month' AND YEAR(order_date) = '$year'";
    } elseif ($month != "") {
        $query = "SELECT * FROM orders WHERE status = 'pending' AND MONTH(order_date) = '$month'";
    } elseif ($year != "") {
        $query = "SELECT * FROM orders WHERE status = 'pending' AND YEAR(order_date) = '$year'";
    } else {
        $query = "SELECT * FROM orders WHERE status = 'pending'";
    }
} elseif (isset($_POST['search'])) {
    $search = $_POST['search'];
    // Ensure the search is sanitized to prevent SQL injection
    $search = $conn->real_escape_string($search);
    $query = "SELECT * FROM orders WHERE status = 'pending' AND (mobile LIKE '%$search%' OR user_id LIKE '%$search%' OR orders LIKE '%$search%' OR pickup_address LIKE '%$search%')";
} else {
    $query = "SELECT * FROM orders WHERE status = 'pending'";
}
...
if ($result->num_rows >  0) {
    // Count delivered and picked-up orders
    $delivered_query = "SELECT * FROM orders WHERE status = 'delivered'";
    $delivered_result = $conn->query($delivered_query);
    $total_delivered_orders = $delivered_result->num_rows;

    $pickedup_query = "SELECT * FROM orders WHERE status = 'pickedup'";
    $pickedup_result = $conn->query($pickedup_query);
    $total_pickedup_orders = $pickedup_result->num_rows;

    // Display pending orders in a table
    $orders_table = "<table>";
    $orders_table .= "<tr><th>Order ID</th><th>Mobile</th><th>Price</th><th>Products</th><th>Wash Method</th><th>Pickup Address</th><th>Delivery address</th><th>Order Date</th><th>Update status</th></tr>";
    $order_ids = array();
    $rows = array();
    $products = array();
    $paid_query = "SELECT * FROM orders WHERE status = 'pending'";
    ...
    foreach ($unique_user_ids as $user_id) {
        ...
        $orders_table .= "<td>
                ...
                <form action='' method='post' class='update-status-form'>
                    ...
                    <button type='submit' name='update_status_button' value='pickedup'>Pickedup</button>
                </form>
            </td>";
        $orders_table .= "</tr>";
    }
...
function sortTable(column, order) {
    ...
    rows.sort(function(a, b) {
        ...
        if (column === 'Order Date') { // Assuming date format is YYYY-MM-DD
            aVal = new order_date(aVal); 
            bVal = new order_date(bVal);
        }
